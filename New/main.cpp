...
void initLighting()
{
    ...
    //glMaterialfv(GL_FRONT,GL_SPECULAR,yellow); // light in front side
...
void orbit()
{
    ...
    int i = 0;
    for (i = 0; i < 8; i++) // To make the round path for every planet
    {
        glPushMatrix();
        if (i == 5) // Rotate the jupiter path in 45 digree
        {
        ...
        glScalef(sc[i], sc[i], sc[i]); // Shape of every planet
        glBegin(GL_POINTS);
        double ang1 = 0.0;
        int i = 0;
        for (i = 0; i < 300; i++) // visible the planet paths 
        {
...
void draw(void)
{
    ...
    for (j = 0; j < 4; j++) //astroid belt made
    {
    ...
    double ang1 = 0.0;
    i = 0;
    for (i = 0; i < 50; i++) // saturn planet round circle 
    {
        glVertex2d(cos(ang1), sin(ang1));
        ang1 += angular;
    }
...
void update(int value) {
    ...
    if (r == 1) {
        // For sun reflection in left side 
        ...
        angleMoon += 2; // Speed of Moon
        if (angleMoon > 360) {
            angleMoon -= 360;
        }
        angleEarth += 0.7; // Speed of Earth
        if (angleEarth > 360) {
            angleEarth -= 360;
        }
        angleMercury += 2; // Speed of Mercury
        if (angleMercury > 360) {
            angleMercury -= 360;
        }
        angleVenus += 0.9; // Speed of Venus
        if (angleVenus > 360) {
            angleVenus -= 360;
        }
        angleMars += 0.5; // Speed of Mars
        if (angleMars > 360) {
            angleMars -= 360;
        }
        angleJupiter += 0.2;
...
void my_mouse(int button, int state, int x, int y) // This one off
{
    switch (button) {
